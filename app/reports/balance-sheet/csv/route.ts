...
import { balanceSheetCsv } from "@/lib/report-csv";
...
export async function GET(req: NextRequest) {
  ...
  const csv = balanceSheetCsv(balanceSheet(accounts, lines, asOf), asOfIso);
  return new NextResponse(csv, {
    headers: {
      "Content-Type": "text/csv; charset=utf-8",
      "Content-Disposition": `attachment; filename="balance-sheet-${asOfIso}.csv"`,
    },
  });
}
